 ...
 *  - param1: consoleId.
 *  - param2: vendedor (seller).
 *  - param3: marca (consoleBrand).
 *  - param4: precio (consolePrice).
...
void processNewCommand(char *commandNumber, char *param1, char *param2, char *param3, char *param4, tList *list) {
    ...
    float param4Float = safeStr2float(param4); //Variable precio transformado a float para poder pasarlo al TAD.

    printf("********************\n");
    printf("%s N: console %s seller %s brand %s price %.2f\n", commandNumber, param1, param2, param3, param4Float);

    if (param4Float < 0) { //Error en conversión (o han se ha introducido un precio negativo).
        printf("+ Error: Invalid price value\n");
        return;
    }

    pos = findItem(param1, *list); //findItem devuelve LNULL en listas vacías.
...
void processDeleteCommand(char *commandNumber, char *param1, tList *list) {
    ...
    tPosL pos = findItem(param1, *list); //También actúa como isEmptyList si devuelve LNULL.
    if (pos == LNULL) {
 ...
 *  - commandNumber: Número de comando.
 *  - param1: consoleId.
 *  - param2: pujador (bidder).
 *  - param3: marca (consoleBrand).
 *  - param4: puja (consolePrice).
 *  - list: Lista en la que se guardan los valores (tList).
 *
 * PostCD:
...
void processBidCommand(char *commandNumber, char *param1, char *param2, char *param3, tList *list) {
    ...
    float bidPrice = safeStr2float(param3); //Variable precio como float para poder pasarlo al TAD y hacer comparaciones.
 ...
 * Entradas:
 *   - commandNumber: Número de comando.
 *   - param1: consoleId de la consola sobre la que se puja.
 *   - list: Lista en la que se encuentra la consola (tList).
 * PostCD:
